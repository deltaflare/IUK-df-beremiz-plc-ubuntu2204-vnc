...
RUN git clone https://github.com/beremiz/beremiz \
    && git clone https://github.com/beremiz/matiec

# COPY patches/requirements.txt /root/Beremiz/beremiz/requirements.txt
RUN virtualenv ~/Beremiz/venv \
    && ~/Beremiz/venv/bin/pip install -U attrdict3 \
    && ~/Beremiz/venv/bin/pip install -r ~/Beremiz/beremiz/requirements.txt

# install MatIEC Compiler
WORKDIR /root/Beremiz/matiec
RUN autoreconf -i \
    && ./configure \
    && make
# end install MatIEC Compiler
